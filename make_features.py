...
def _merge_holidays(base: pd.DataFrame, holidays_events: Optional[pd.DataFrame]) -> pd.DataFrame:
    ...
    if (
        holidays_events is None
        or holidays_events.empty
        or not required.intersection(set(holidays_events.columns))
    ):
        base["is_holiday"] = 0
    ...
    nat = (
        hol[hol.get("locale", pd.Series(dtype=str)).str.lower() == "national"][["date"]]
        .drop_duplicates()
        .copy()
    )
    ...
    if "state" in df.columns and "locale_name" in reg.columns:
        reg_simple = (
            reg[["date", "locale_name"]].drop_duplicates().rename(columns={"locale_name": "state"})
        )
    ...
    if "city" in df.columns and "locale_name" in loc.columns:
        loc_simple = (
            loc[["date", "locale_name"]].drop_duplicates().rename(columns={"locale_name": "city"})
        )
    ...
    df["is_holiday"] = (
        df["is_holiday_national"] | df["is_holiday_regional"] | df["is_holiday_local"]
    ).astype(int)
    return df
...
def _merge_transactions(base: pd.DataFrame, transactions: Optional[pd.DataFrame]) -> pd.DataFrame:
    ...
    if "store_nbr" in tr.columns:
        out = base.merge(
            tr[["date", "store_nbr", "transactions"]], on=["date", "store_nbr"], how="left"
        )
    else:
...
def _merge_oil(
    base: pd.DataFrame, oil: Optional[pd.DataFrame], price_col: str = "dcoilwtico"
) -> pd.DataFrame:
...
def _add_txn_oil_lags_rolls(
    ...
    if txn_col in out.columns:
        for lag in lags:
            out[f"{txn_col}_lag_{lag}"] = g[txn_col].shift(lag)
        for window in roll_windows:
            shifted = g[txn_col].shift(1)
            out[f"{txn_col}_rollmean_{window}"] = shifted.rolling(window, min_periods=1).mean()
            out[f"{txn_col}_rollstd_{window}"] = shifted.rolling(window, min_periods=1).std()
    if oil_col in out.columns:
        for lag in lags:
            out[f"{oil_col}_lag_{lag}"] = g[oil_col].shift(lag)
        for window in roll_windows:
            shifted = g[oil_col].shift(1)
            out[f"{oil_col}_rollmean_{window}"] = shifted.rolling(window, min_periods=1).mean()
            out[f"{oil_col}_rollstd_{window}"] = shifted.rolling(window, min_periods=1).std()
    return out
...
def _add_onpromotion_features(
    ...
    g = out.groupby(group_cols, sort=False)
    for lag in lags:
        out[f"{promo_col}_lag_{lag}"] = g[promo_col].shift(lag)
    for window in roll_windows:
        shifted = g[promo_col].shift(1)
        out[f"{promo_col}_rollmean_{window}"] = shifted.rolling(window, min_periods=1).mean()
        out[f"{promo_col}_rollstd_{window}"] = shifted.rolling(window, min_periods=1).std()
    return out
    ...
    for i in range(len(dates)):
        if is_h[i]:
            ...
            if prev_idx == -1:
                ...
                prev_days.append(
                    (dates[i] - dates[prev_idx]).astype("timedelta64[D]").astype(float)
                )
    # следующий праздник
    next_idx = -1
...
def _add_group_lags_rolls(
    ...
    for lag in lags:
        df[f"{target_col}_lag_{lag}"] = g[target_col].shift(lag)
    shifted = g[target_col].shift(1)
    for window in roll_windows:
        df[f"{target_col}_rollmean_{window}"] = shifted.rolling(window, min_periods=1).mean()
        df[f"{target_col}_rollstd_{window}"] = shifted.rolling(window, min_periods=1).std()
    return df
...
def make_features(
    ...
    def _is_oil(x: Optional[pd.DataFrame]) -> bool:
        return isinstance(x, pd.DataFrame) and (
            "dcoilwtico" in x.columns or "oil_price" in x.columns
        )

    def _is_holidays(x: Optional[pd.DataFrame]) -> bool:
        return isinstance(x, pd.DataFrame) and (
            "locale" in x.columns or "type" in x.columns or "transferred" in x.columns
        )
    ...
    df = _add_txn_oil_lags_rolls(
        df, group_cols, txn_col="transactions", oil_col="oil_price", lags=[7], roll_windows=[7, 30]
    )
    # Признаки по промо-акциям (из train/test: onpromotion)
    df = _add_onpromotion_features(
        df, group_cols, promo_col="onpromotion", lags=[1, 7, 14, 28], roll_windows=[7, 30]
    )
