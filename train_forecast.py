...
def parse_args():
    ...
    p.add_argument(
        ...
        type=int,
        default=90,
        help="Отбирать топ-N по сумме продаж за последние N дней",
    )
    p.add_argument("--valid_days", type=int, default=28)
    p.add_argument(
        "--cv_folds", type=int, default=0, help="Количество временных фолдов (0 = без CV)"
    )
    ...
    p.add_argument(
        ...
        type=float,
        default=[],
        help="Список квантилей для обучения (например 0.5 0.9)",
    )
...
def merge_config_with_args(args: argparse.Namespace) -> argparse.Namespace:
    ...
    try:
        ...
        for key, val in data.items():
            ...
            if (
                cur in (None, [], "") or (isinstance(cur, (int, float)) and cur == 0)
            ) and val is not None:
...
def time_folds(
    dates: pd.Series, valid_days: int, cv_folds: int, cv_step_days: int
...
def main():
    ...
    bar = tqdm(
        train_pairs,
        desc=f"Training per-SKU ({args.lgb_objective})",
        unit="pair",
        total=total,
        smoothing=0.3,
        mininterval=0.5,
    ...
    for idx, (store, fam) in enumerate(bar, start=1):
        ...
        if args.lgb_objective == "l1":
            ...
            lgb_params.update(
                objective="tweedie", tweedie_variance_power=args.tweedie_variance_power
            )
        else:  # poisson
        ...
        try:
            feat_names = (
                ...
                else list(model.booster_.feature_name())
            )
        except Exception:
        ...
        if args.quantiles:
            ...
            iter_quants = (
                ...
                if len(quants) <= 1
                    ...
                    quants, desc=f"Quantiles {store}/{fam}", unit="q", leave=False
                )
            )
    ...
    if metrics:
        ...
        with open(
            os.path.join(args.warehouse_dir, "summary_metrics.txt"), "w", encoding="utf-8"
        ) as f:
