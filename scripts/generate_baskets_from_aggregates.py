...
def load_data():
    ...
    tx = (
        pd.read_csv(tx_path)
        if tx_path.exists()
        else pd.DataFrame(columns=["date", "store_nbr", "transactions"])
    )
    if not tx.empty:
...
def generate_for_day(
    group, n_tx, fuel_anchor=False, fuel_names=("FUEL", "GASOLINE", "DIESEL"), anchor_prob=0.45
):
    """
    ...
    fuel_present = [
        idx
        for idx, fam_name in enumerate(fam)
        if any(tag in fam_name.upper() for tag in fuel_names)
    ]
...
def main(min_tx=50, max_tx=1200, fuel_anchor=True):
    ...
    if not tx.empty:
        base = base.merge(
            tx.groupby(["store_nbr", "date"], as_index=False)["transactions"].sum(),
            on=["store_nbr", "date"],
            how="left",
        )
    else:
...
if __name__ == "__main__":
    ...
    ap.add_argument(
        "--no_fuel_anchor",
        action="store_true",
        help="не форсировать присутствие топлива в части чеков",
    )
    args = ap.parse_args()
