...
def main() -> None:
    ...
    trans = (
        pd.read_csv(raw / "transactions.csv", parse_dates=["date"])
        if (raw / "transactions.csv").exists()
        else None
    )
    oil = pd.read_csv(raw / "oil.csv", parse_dates=["date"]) if (raw / "oil.csv").exists() else None
    hol = (
        pd.read_csv(raw / "holidays_events.csv", parse_dates=["date"])
        if (raw / "holidays_events.csv").exists()
        else None
    )
    ...
    for i, (s, f) in enumerate(pairs, 1):
        ...
        with tempfile.TemporaryDirectory() as td:
            ...
            (trans if trans is not None else pd.DataFrame(columns=["date", "transactions"])).to_csv(
                tdp / "transactions.csv", index=False
            )
            (oil if oil is not None else pd.DataFrame(columns=["date", "dcoilwtico"])).to_csv(
                tdp / "oil.csv", index=False
            )
            (hol if hol is not None else pd.DataFrame(columns=["date", "locale"])).to_csv(
                tdp / "holidays_events.csv", index=False
            )
            (stores if stores is not None else pd.DataFrame(columns=["store_nbr"])).to_csv(
                tdp / "stores.csv", index=False
            )
            cmd = [
                "python",
                "train_forecast.py",
                ...
                str(tdp / "train.csv"),
                "--transactions",
                str(tdp / "transactions.csv"),
                "--oil",
                str(tdp / "oil.csv"),
                "--holidays",
                str(tdp / "holidays_events.csv"),
                "--stores",
                str(tdp / "stores.csv"),
                "--models_dir",
                "models",
