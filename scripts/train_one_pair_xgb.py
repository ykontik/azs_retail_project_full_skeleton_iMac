...
def main():
    p = argparse.ArgumentParser("Train single (store, family) XGBoost model")
    ...
    if df.empty:
        raise SystemExit(f"Нет данных для пары ({args.store}, {args.family}).")
    y = df["sales"].values
    X = df[feat_cols]
    ...
    print(f"OK XGB: ({args.store}, {args.family})  MAE={mae:.3f}  MAPE={mmape:.2f}%")

    Path(args.models_dir).mkdir(parents=True, exist_ok=True)
    safe_family = _sanitize_family_name(args.family)
    out = Path(args.models_dir) / f"{args.store}__{safe_family}__xgb.joblib"
    dump(model, out)
    print("Saved →", out)
