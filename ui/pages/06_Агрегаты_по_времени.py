...
def load_raw():
    ...
    oil = (
        pd.read_csv(DATA_DIR / "oil.csv", parse_dates=["date"])
        if (DATA_DIR / "oil.csv").exists()
        else None
    )
    hol = (
        pd.read_csv(DATA_DIR / "holidays_events.csv", parse_dates=["date"])
...
max_models = st.sidebar.slider(
    "Ограничить кол-во моделей", min_value=10, max_value=300, value=100, step=10
)
...
all_stores = (
    sorted(Xfull["store_nbr"].dropna().astype(int).unique().tolist())
    if "store_nbr" in Xfull.columns
    else []
)
all_fams = (
    sorted(Xfull["family"].dropna().astype(str).unique().tolist())
    if "family" in Xfull.columns
    else []
)
f1, f2 = st.columns(2)
with f1:
...
for i, mp in enumerate(models[:total], start=1):
    ...
    df_pair = (
        Xfull[(Xfull["store_nbr"] == store) & (Xfull["family"] == fam)].copy().sort_values("date")
    )
    if df_pair.empty:
        continue
    ...
    for _, row in g.iterrows():
        ...
        acc[key]["y_true"] += (
            float(row["y_true"]) if agg_fn == "sum" else float(row["y_true"])
        )  # mean уже учтён на шаге выше
        acc[key]["y_pred"] += (
            float(row["y_pred"]) if agg_fn == "sum" else float(row["y_pred"])
        )  # mean уже учтён на шаге выше
...
piv = (
    dfm.pivot_table(index=dim_key, columns="period", values=metric, aggfunc="mean")
    .fillna(0.0)
    .round(2)
)

st.subheader(
    f"Тепловая карта: {metric} ({'сумма' if agg_fn=='sum' else 'среднее'}) по {dimension.lower()} и периодам"
)
st.caption(
    "Ось Y — %s; ось X — %s. Подсказка: горизонтальная прокрутка для длинных периодов"
    % (
        "магазины" if dimension == "По магазинам" else "семейства",
        "недели" if period == "Неделя" else "месяцы",
    )
)
st.dataframe(
    piv.style.format("{:.2f}").background_gradient(cmap="YlOrRd"), use_container_width=True
)
st.download_button(
